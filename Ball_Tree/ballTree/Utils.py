...
def checkResult(augGmmResult, gmmResult):
    ...
    #if np.array_equal(sorted(augGmmResult),sorted(gmmResult)).all():
    val = (augGmmResult == np.array(gmmResult))
    if val.all():
        print("array equal")
    else:
...
def makeBlobs_data(numberofSample):
    ...
    X, Y = make_blobs(n_samples=numberofSample,centers=1000, n_features=2,
                  random_state=42, cluster_std=0.1)
    #dataset=pd.read_csv(r'Dataset\makeblobs\largeMakeBlobs.csv', nrows=numberofSample)
    #X1 = X.iloc[:, :].values
    return X
