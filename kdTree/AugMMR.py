...
def CalculateBound(kdt,level,arity,q,lambda_score,S,simmap,indexMap,candNodes,levelMatrix,dismatrix):
    lastItem = None
    if len(S) != 0:
        lastItem = S[-1]
    ...
    for node in candNodes:
        ...
        if lastItem is not None:
            ...
            recordId = kdt.documentMap[tuple(lastItem)][level]
...
def DivGetBatch(kdt,L,arity,q,lambda_score,S,simmap,indexMap,levelMatrix,dismatrix):
    candNodes = [kdt.root.left,kdt.root.right]

    for level in range(1,L+1):
        lbMMRList, ubMMRList = CalculateBound(kdt,level,arity,q,lambda_score,S,simmap,indexMap,candNodes,levelMatrix,dismatrix) #calculateBound(iTree,C,indexMap,iTreeArray,l)
...
def AugMMR(kdt,L,arity,q,lambda_score,k,indexMap):
    nodes = kdt.traverse(kdt.root)

    levelMatrix = kdt.createLevelMatrix(nodes, L + 1)
    dismatrix = kdt.createDistanceMatrix(levelMatrix, L + 1)
    ...
    for i in range (k):
        mmr = -100000000
        candR = DivGetBatch(kdt,L,arity,q,lambda_score,S,simmap,indexMap,levelMatrix,dismatrix)
