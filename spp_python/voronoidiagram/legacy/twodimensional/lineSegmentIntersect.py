...
def lineSegmentIntersect(XY1 = None,XY2 = None): 
...
#-------------------------------------------------------------------------------
    ...
    validateattributes(XY1,np.array(['numeric']),np.array(['2d','finite']))
    validateattributes(XY2,np.array(['numeric']),np.array(['2d','finite']))
    n_rows_1,n_cols_1 = XY1.shape
    n_rows_2,n_cols_2 = XY2.shape
    if n_cols_1 != 4 or n_cols_2 != 4:
        raise Exception('Arguments must be a Nx4 matrices.')
    
    ### Prepare matrices for vectorized computation of line intersection points.
#-------------------------------------------------------------------------------
    X1 = np.matlib.repmat(XY1(:,1),1,n_rows_2)
    X2 = np.matlib.repmat(XY1(:,3),1,n_rows_2)
    Y1 = np.matlib.repmat(XY1(:,2),1,n_rows_2)
    Y2 = np.matlib.repmat(XY1(:,4),1,n_rows_2)
    XY2 = np.transpose(XY2)
    X3 = np.matlib.repmat(XY2(1,:),n_rows_1,1)
    X4 = np.matlib.repmat(XY2(3,:),n_rows_1,1)
    Y3 = np.matlib.repmat(XY2(2,:),n_rows_1,1)
    Y4 = np.matlib.repmat(XY2(4,:),n_rows_1,1)
    X4_X3 = (X4 - X3)
    Y1_Y3 = (Y1 - Y3)
    Y4_Y3 = (Y4 - Y3)
    X1_X3 = (X1 - X3)
    X2_X1 = (X2 - X1)
    Y2_Y1 = (Y2 - Y1)
    numerator_a = np.multiply(X4_X3,Y1_Y3) - np.multiply(Y4_Y3,X1_X3)
    numerator_b = np.multiply(X2_X1,Y1_Y3) - np.multiply(Y2_Y1,X1_X3)
    denominator = np.multiply(Y4_Y3,X2_X1) - np.multiply(X4_X3,Y2_Y1)
    u_a = numerator_a / denominator
    u_b = numerator_b / denominator
    # Find the adjacency matrix A of intersecting lines.
    INT_X = X1 + np.multiply(X2_X1,u_a)
    INT_Y = Y1 + np.multiply(Y2_Y1,u_a)
    INT_B = np.logical_and(np.logical_and(np.logical_and((u_a >= 0),(u_a <= 1)),(u_b >= 0)),(u_b <= 1))
    PAR_B = denominator == 0
    COINC_B = (numerator_a == np.logical_and(0,numerator_b) == np.logical_and(0,PAR_B))
    # Arrange output.
    out.intAdjacencyMatrix = INT_B
    out.intMatrixX = np.multiply(INT_X,INT_B)
    out.intMatrixY = np.multiply(INT_Y,INT_B)
